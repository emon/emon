 ...
 * started:   2001/07/10
 *
 * $Id: jpegcapt.c,v 1.1 2008/09/26 15:10:34 emon Exp $
 */
...
CAPTURE* capt_init(opt_t *opt);
int capt_start(CAPTURE *capt, opt_t *opt);
...
capt_init(opt_t *opt)
{
	...
	capt = CaptureOpen(opt->dev);
	CaptureSetSize(capt, opt->width, opt->height);
	CaptureSetFormat(capt, CAPTURE_FORMAT_RGB24);
	CaptureSetInput(capt, opt->line);
	CaptureSetFieldMode(capt, opt->field);
	
	opt->fps_dev = CaptureGetFps(capt);
	d2_printf("\nfps_dev=%d, field mode=%d",
		  opt->fps_dev, CaptureGetFieldMode(capt));
	return capt;
}
...
int
capt_capt(CAPTURE *capt, opt_t *opt)
{
	int frame_size;
	...
	for (frame_num = 0; opt->max_frame == 0 || frame_num < opt->max_frame; frame_num++, ts += opt->freq){
...
#ifdef WAIT_CAPTURE
		if (wait_proper_time(&start, fps, frame_num, wait_flag) < 0){
#else			
		...
		gettimeofday(&c, NULL);
#ifdef WAIT_CAPTURE
		CaptureGetImageSync(capt, frame_buf, 1);
#else
		CaptureGetImageAsync(capt, frame_buf, 1);
//		CaptureGetImageNoWait(capt, frame_buf, 1);
#endif
		gettimeofday(&b, NULL);
		len = jpeg_encode(frame_buf, jpeg_buf, w, h, q);
		gettimeofday(&a, NULL);
...
void
capt_end(CAPTURE *capt, opt_t *opt)
/*
...
{
	int ch;
	char *opts = "C:R:w:h:d:l:f:q:s:n:D:S:";
	char *helpmes =
		"usage : %s\n"
		" options\n"
		" <general>\n"
		...
		"  -l <c>   : input video line (0-5, C:component, S:S-vide)\n"
	...
	if ((temp = getenv("JPEGCAPT_DEV")) != NULL)
		opt->dev = temp;
	if ((temp = getenv("JPEGCAPT_LINE")) != NULL)
		opt->line = atoi(temp);
	...
	while ((ch = getopt(argc, argv, opts)) != -1){
		...
		case 'l':
			...
			case 'c':
			case 'C':
				opt->line = CAPTURE_INPUT_DEV0;
				break;
			case '1':
				opt->line = CAPTURE_INPUT_DEV1;
				break;
			case '2':
				opt->line = CAPTURE_INPUT_DEV2;
				break;
			case '3':
				opt->line = CAPTURE_INPUT_DEV3;
				break;
			case '4':
				opt->line = CAPTURE_INPUT_DEV4;
				break;
			case '5':
			case 's':
			case 'S':
				opt->line = CAPTURE_INPUT_DEV5;
				break;
			default:
				printf("\nInvalid capture line: %s\n", optarg);
				return -1;
			}
			break;
		case 'f':
	...
	d1_printf("\nclock=%d, freq=%d, fps=%d, size=(%d,%d)\n"
		  "input device=%s, input line=%d, capture field=%d",
		  opt->clock, opt->freq, opt->fps_opt, opt->width, opt->height,
		  opt->dev, opt->line, opt->field
		  );
	return 0;
}
